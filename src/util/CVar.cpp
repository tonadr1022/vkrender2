...
template <>
CVarArray<i32>& CVarSystemImpl::get_cvar_array() {
  return int_cvars_;
}
...
template <typename T>
T* CVarSystemImpl::get_cvar_current(u32 hash) {
...
template <typename T>
void CVarSystemImpl::set_cvar_current(u32 hash, const T& value) {
  CVarParameter* param = get_cvar(hash);
  if (param) {
...
void CVarSystemImpl::draw_imgui_edit_cvar_parameter(CVarParameter* p, float text_width) {
  const bool is_read_only = static_cast<u32>(p->flags) & static_cast<u32>(CVarFlags::EditReadOnly);
