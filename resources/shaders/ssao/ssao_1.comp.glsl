...
void main() {
    ...
    // float depth = texture(vk2_sampler2D(pc.depth_img, 0), uv).r;
    ...
    for (int i = 0; i < kernel_size; i++) {
        ...
        // TODO: learn
        float range_check = smoothstep(0.0, 1.0, pc.radius / abs(frag_pos.z - sample_view_z));

        occlusion += (sample_view_z >= sample_pos.z + 0.025 ? 1.0 : 0.0) * range_check;
    }
