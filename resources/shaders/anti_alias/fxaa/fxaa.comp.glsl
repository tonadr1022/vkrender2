...
#define STORE(x) imageStore(vk2_get_storage_img(image2D, pc.output_img), tex_coord, x)
...
void main() {
    ...
    ivec2 img_size = imageSize(vk2_get_storage_img(image2D, pc.output_img));
    if (any(greaterThanEqual(tex_coord, img_size))) {
        return;
    }
    vec2 texel_size = 1.0 / vec2(img_size);
    vec2 uv = vec2(tex_coord) * texel_size;
